...
def make_best_move(state, is_max_player):
    ...
    for move in state.getAllPossibleMoves():
        state.makeMove(move)
        evaluation = minimax(state, 100, MIN_INT, MAX_INT, not is_max_player)
...
def minimax(state, depth, alpha, beta, is_max_player):
    ...
    if is_max_player:
        ...
        for move in state.getAllPossibleMoves():
            state.makeMove(move)
            evaluation = minimax(state, depth - 1, alpha, beta, False)
        ...
        for move in state.getAllPossibleMoves():
            state.makeMove(move)
            evaluation = minimax(state, depth - 1, alpha, beta, True)
            eval_list.append(evaluation)
            state.undoMove(move)        
